 ...
 * (2013-2019) using a split-panel architecture, with the 112 provinces as the
 * underlying administrative units (bolivia112).
 * * * FEATURES:
 * 1. Spatial Clipping: Analysis restricted to Bolivia (LSIB Boundary).
 * 2. Province Boundaries: Overlay of the 112 Bolivian provinces.
 * 3. Split-Panel: Side-by-side comparison of 2013 vs 2019.
 * 4. Interactive Inspector: Click to calculate pixel-level changes.
 * 5. Contextual Metadata: Full data dictionary and usage instructions.
 * =================================================================================
 */
...
// -----------------------------------------------------------------------------
// PROVINCE BOUNDARY ASSET (112 provinces)
// -----------------------------------------------------------------------------
// NOTE: A province boundary asset (the 112 Bolivian provinces) must be uploaded
// to Earth Engine for this overlay to work. Convert
// maps/bolivia112provincesOpt.geojson (or ...Full.geojson) to a
// shapefile/FeatureCollection and upload it as a GEE asset, then replace the
// PLACEHOLDER path below with your own asset id (e.g.
// "projects/<your-project>/assets/bolivia112provinces"). Expected feature
// properties: prov_id, prov, dep, dep_id, shapeName, COORD_X, COORD_Y.
var provinces = ee.FeatureCollection(
  "projects/REPLACE_WITH_YOUR_PROJECT/assets/bolivia112provinces"
);
...
sidebar.add(ui.Label({
  value: 'Population, Nightlights, Landcover, and GDP across 112 provinces (2013-2019)',
...
sidebar.add(ui.Label({
  value: 'Geospatial development is a complex interplay between people, economic activity, and the environment. ' +
         'This tool restricts analysis to the Bolivian administrative boundary and overlays the 112 provinces ' +
         '(bolivia112) to explore these dynamics.',
...
// --- RESTORED: CITATION ---
sidebar.add(ui.Label({
  value: 'Mendez (2026) Exploring Regional Development from Outer Space. ' +
         'bolivia112 (112 provinces) web application.',
...
function updateMapLayers(layerType) {
  ...
  // --- Province boundaries overlay (112 provinces) ---
  // NOTE: Requires the province boundary asset to be uploaded to Earth Engine
  // (see SECTION 0). Paints the outline of the 112 Bolivian provinces.
  var provImg = ee.Image().paint(provinces, 1, 1);
  var provStyle = {palette: 'yellow'};

  var provLeft = ui.Map.Layer(provImg, provStyle, '112 Provinces');
  var provRight = ui.Map.Layer(provImg, provStyle, '112 Provinces');

  mapLeft.layers().add(provLeft);
  mapRight.layers().add(provRight);
...
mapLeft.onClick(function(coords) {
  ...
  check.size().evaluate(function(size) {
    ...
    // Index 3: above the Bolivia border (0/1), the data layer, and the
    // province boundary overlay (2). Keeps the selected-point marker on top.
    mapLeft.layers().set(3, dotLeft);
    mapRight.layers().set(3, dotRight);
